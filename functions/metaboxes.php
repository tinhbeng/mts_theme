...
function mts_inner_sidebar_metabox($post) {
    ...
    */
    ...
    echo '<option value="" '.selected('', $custom_sidebar).'>-- '.__('Default', 'best' ).' --</option>';
    
    // Exclude built-in sidebars
    $hidden_sidebars = array('sidebar', 'footer-top', 'footer-top-2', 'footer-top-3', 'footer-top-4', 'widget-header','shop-sidebar', 'product-sidebar');    
    ...
    echo '<option value="mts_nosidebar" '.selected('mts_nosidebar', $custom_sidebar).'>-- '.__('No sidebar --', 'best' ).'</option>';
    echo '</select><br />';
    
    // Select single layout (left/right sidebar)
    echo '<div class="mts_sidebar_location_fields">';
    echo '<label for="mts_sidebar_location_default" style="display: inline-block; margin-right: 20px;"><input type="radio" name="mts_sidebar_location" id="mts_sidebar_location_default" value=""'.checked('', $sidebar_location, false).'>'.__('Default side', 'best' ).'</label>';
    echo '<label for="mts_sidebar_location_left" style="display: inline-block; margin-right: 20px;"><input type="radio" name="mts_sidebar_location" id="mts_sidebar_location_left" value="left"'.checked('left', $sidebar_location, false).'>'.__('Left', 'best' ).'</label>';
    echo '<label for="mts_sidebar_location_right" style="display: inline-block; margin-right: 20px;"><input type="radio" name="mts_sidebar_location" id="mts_sidebar_location_right" value="right"'.checked('right', $sidebar_location, false).'>'.__('Right', 'best' ).'</label>';
    echo '</div>';
...
function mts_inner_posttemplate_metabox($post) {
    ...
    */
    ...
    echo '<option value="" '.selected('', $posttemplate).'>'.__('Default Post Template', 'best' ).'</option>';
    echo '<option value="parallax" '.selected('parallax', $posttemplate).'>'.__('Parallax Template', 'best' ).'</option>';
    echo '<option value="zoomout" '.selected('zoomout', $posttemplate).'>'.__('Zoom Out Effect Template', 'best' ).'</option>';
    echo '</select><br />';
}
...
function mts_add_postheader_metabox() {
    ...
    foreach ($screens as $screen) {
        add_meta_box(
            ...
            __('Header Animation', 'best' ),    // title
...
function mts_inner_postheader_metabox($post) {
    ...
    */
    ...
    echo '<option value="" '.selected('', $postheader).'>'.__('None', 'best' ).'</option>';
    echo '<option value="parallax" '.selected('parallax', $postheader).'>'.__('Parallax Effect', 'best' ).'</option>';
    echo '<option value="zoomout" '.selected('zoomout', $postheader).'>'.__('Zoom Out Effect', 'best' ).'</option>';
    echo '</select><br />';
}
    ...
    */
    ...
    foreach ( $registered_nav_menus as $location => $description ) {
        ...
        if ( isset( $locations[ $location ] ) ) {
            ...
            echo '<p>' . __('No menu assigned to this location.', 'best' ) . '</p>';
        }
    }
}
