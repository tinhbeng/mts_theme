...
class mts_popular_posts_widget extends WP_Widget {
	...
	public function __construct() {
		parent::__construct(
			...
			sprintf( __('%sPopular Posts', 'best' ), MTS_THEME_WHITE_LABEL ? '' : 'MTS ' ),
			array( 'description' => __( 'Displays most Popular Posts with Thumbnail.', 'best' ) )
		);
	}
 	...
 	public function form( $instance ) {
		...
		$title = isset( $instance[ 'title' ] ) ? $instance[ 'title' ] : __( 'Popular Posts', 'best' );
		...
		<p>
	       <label for="<?php echo $this->get_field_id( 'days' ); ?>"><?php _e( 'Popular limit (days):', 'best' ); ?>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'qty' ); ?>"><?php _e( 'Number of Posts to show', 'best' ); ?></label>
		...
		<p>
	       <label for="<?php echo $this->get_field_id( 'title_length' ); ?>"><?php _e( 'Title Length:', 'best' ); ?>
		...
		<p>
			<label for="<?php echo $this->get_field_id("show_thumb3"); ?>">
				...
				<?php _e( 'Show Thumbnails', 'best' ); ?>
			</label>
		</p>

		<p>
			<label for="<?php echo $this->get_field_id('box_layout'); ?>"><?php _e('Posts layout:', 'best' ); ?></label>
			<select id="<?php echo $this->get_field_id('box_layout'); ?>" name="<?php echo $this->get_field_name('box_layout'); ?>">
				<option value="horizontal-small" <?php selected($box_layout, 'horizontal-small', true); ?>><?php _e('Horizontal', 'best' ); ?></option>
				<option value="vertical-small" <?php selected($box_layout, 'vertical-small', true); ?>><?php _e('Vertical', 'best' ); ?></option>
			</select>
		</p>
		...
		<p>
			<label for="<?php echo $this->get_field_id("comment_num"); ?>">
				...
				<?php _e( 'Show number of comments', 'best' ); ?>
			</label>
		</p>
		...
		<p>
			<label for="<?php echo $this->get_field_id("show_excerpt"); ?>">
				...
				<?php _e( 'Show excerpt', 'best' ); ?>
			</label>
		</p>
		
		<p>
	       <label for="<?php echo $this->get_field_id( 'excerpt_length' ); ?>"><?php _e( 'Excerpt Length:', 'best' ); ?>
			...
			<?php echo $open_li_item; ?>
				...
				<div class="post-img">
					<a href="<?php echo esc_url( get_the_permalink() ); ?>" title="<?php echo esc_attr( get_the_title() ); ?>">
						<?php if ( has_post_thumbnail() ) { ?>
							<?php the_post_thumbnail( 'best-' . $thumbnail, array( 'title' => '' ) ); ?>
						<?php } else { ?>
							<img class="wp-post-image" src="<?php echo get_template_directory_uri() . '/images/nothumb-best-' . $thumbnail . '.png'; ?>" alt="<?php echo esc_attr( get_the_title() ); ?>"/>
						<?php } ?>
					</a>
				</div>
				<?php endif; ?>
				<div class="post-data">
					<div class="post-data-container">
						...
						<div class="post-info">
							<?php if ( $date == 1 ) : ?>
								<span class="thetime updated"><i class="fa fa-clock-o"></i> <?php the_time( 'M j, Y' ); ?></span>
							<?php endif; ?>
