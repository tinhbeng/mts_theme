...
class Facebook_Like_Widget extends WP_Widget {
	...
	{
		$widget_ops = array('classname' => 'facebook_like', 'description' => __('Add Facebook Like Box.', 'best' ));

		$control_ops = array('id_base' => 'facebook-like-widget');

		parent::__construct('facebook-like-widget', sprintf( __('%sFB Like Box', 'best' ), MTS_THEME_WHITE_LABEL ? '' : 'MTS ' ), $widget_ops, $control_ops);
	}
	...
	{
		$defaults = array('title' => __('Find us on Facebook', 'best' ), 'page_url' => '', 'width' => '300', 'show_faces' => 'on', 'show_stream' => false, 'show_header' => 'on');
		...
		<p>
			<label for="<?php echo $this->get_field_id('page_url'); ?>"><?php _e('Facebook Page URL', 'best' ); ?>:</label>
		...
		<p>
			<label for="<?php echo $this->get_field_id('width'); ?>"><?php _e('Width', 'best' ); ?>:</label>
		...
		<p>
			...
			<label for="<?php echo $this->get_field_id('show_faces'); ?>"><?php _e('Show faces', 'best' ); ?></label>
		</p>
		...
		<p>
			...
			<label for="<?php echo $this->get_field_id('show_stream'); ?>"><?php _e('Show stream', 'best' ); ?></label>
		</p>
		...
		<p>
			...
			<label for="<?php echo $this->get_field_id('show_header'); ?>"><?php _e('Show facebook header', 'best' ); ?></label>
		</p>
	<?php
	}
}
