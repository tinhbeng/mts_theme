...
class NHP_Options_upload extends NHP_Options{	
	...
	*/
	function render(){
		...
		echo ' <a href="javascript:void(0);" class="nhp-opts-upload button-secondary"'.$upload.' rel-id="'.$this->field['id'].'">'.__('Browse', 'best' ).'</a>';
		echo ' <a href="javascript:void(0);" class="nhp-opts-upload-remove"'.$remove.' rel-id="'.$this->field['id'].'">'.__('Remove Upload', 'best' ).'</a>';
	...
	*/
	function enqueue(){
		...
		wp_localize_script('nhp-opts-field-upload-js', 'nhp_upload', array(
			...
			'title' => __( 'Select Image', 'best' ),
			'buttonText' => __( 'Select Image', 'best' )
		));
